...
\end{tikzpicture}
\caption{Conditional density of position based on data $z_1$ and $z_2$ from \cite{Maybeck79}.} \label{fig:combination}
\end{figure}
...
\begin{tikzpicture}[auto, thick, node distance=1.5cm,>=latex']
    ...
    \draw [->] (phi1) -- node[pos=0.27, name=h-phi] {} node[] {$\mathbf{x}^-_k$} (H1);
    \draw [->] (delay1) -- node[] {$\mathbf{x}^-_{k-1}$} node[] {} (phi1);
...
\node [block, align=center] (init) {Initialisation of parameters \\ $\mathbf{P}_{0}, \mathbf{x}_{0}, \mathbf{H}_{0}, \bm{\Phi}_{0}, \mathbf{Q}_{0}, \mathbf{R}_{0},$};
\node [block, align=center, below of=init, node distance=3.2cm] (predict) {Time update (predict) \\ Compute \emph{a priori} estimate: \\ $\hat{\mathbf{x}}^-_k = \bm{\Phi}_{k-1}\hat{\mathbf{x}}_{k-1}+\mathbf{B}_{k-1}\mathbf{u}_{k-1}$ \\ Compute \emph{a priori} error covariance: \\ $\mathbf{P}^-_{k} = \bm{\Phi}_{k-1} \mathbf{P}_{k-1} \bm{\Phi}^T_{k-1} + \mathbf{Q}_{k-1}$};
\node [block, align=center, below of=predict] (update) {Measurement update (correct) \\ Compute Kalman gain: \\ $\mathbf{K}_{k} = \mathbf{P}^-_k \mathbf{H}^T_k[\mathbf{H}_k \mathbf{P}^-_k \mathbf{H}^T_k + \mathbf{R}_k]^{-1}$ \\ Compute \emph{a posteriori} estimate: \\ $\hat{\mathbf{x}}_k = \hat{\mathbf{x}}^-_k + \mathbf{K}_{k}[\mathbf{z}_k-\mathbf{H}_{k}\hat{\mathbf{x}}^-_k]$ \\ Update error covariance: \\ $\mathbf{P}_{k} = [\mathbf{I} - \mathbf{K}_{k}\mathbf{H}_{k}]\mathbf{P}^-_{k}$};
\node [output, below of=update, node distance=3cm, name=output] {Output};
\node [output, below of=update, node distance=2.5cm, name=help1] {};
\node [output, right of=help1, node distance=4.8cm, name=help2] {};
\node [output, below of=init, node distance=1.25cm, name=help4] {};
\node [output, right of=help4, node distance=4.8cm, name=help3] {};

\draw [draw,->, thick, align=left] (init) -- (predict);
\draw [draw,->, thick, align=left] (predict) -- (update);
\draw [draw,->, thick, align=left] (update) -- node [label={left:Output}]{} (output);
\draw [draw,-, thick] (help1) -- (help2);
\draw [draw,-, thick] (help2) -- (help3);
\draw [draw,->, thick] (help3) -- (help4);

\end{tikzpicture}
\caption{Operation cycle of Kalman filter algorithm illustrating predict and correct behaviour.} \label{fig:kalman_filter_cycle}
\end{figure}
...
\begin{equation}\label{eq:time_dynamical_system_plant_extended}
  \mathbf{x}_k = \bm{\phi}_{k-1}(\mathbf{x}_{k-1})+\mathbf{w}_{k-1}, \quad \mathbf{w}_{k} \sim \mathcal{N}(0,\mathbf{Q}_k)\,,
\end{equation}

\noindent
and/or with a non-linear transformation from state variables to measurement variables
...
\noindent
The \emph{functional} $\bm{\phi}_{k-1}$ denotes the \emph{non-linear} transition matrix function that may be time varying. It relates the state at the previous time step $k-1$ to the current time step $k$. The functional $\mathbf{h}_{k}$ denotes a \emph{non-linear} measurement matrix function that relates the state $\mathbf{x}_{k}$ to the measurement $\mathbf{z}_k$ and is possibly time varying, too.

Some non-linear problems can be deemed \emph{quasilinear}, which means that the variation of the non-linear functionals $\bm{\phi}$ and $\mathbf{h}$ are predominantly linear about the value $\mathbf{x}$. That is,

\begin{equation}\label{eq:linear_phi}
  \bm{\phi}_{k}(\mathbf{x} + d \mathbf{x}) \approx \bm{\phi}_{k}(\mathbf{x}) + d \mathbf{x} \left. \frac{\partial \bm{\phi}_{k}(\mathbf{x})}{\partial \mathbf{x}} \right|_{\mathbf{x}}\,,
\end{equation}

\begin{equation}\label{eq:linear_h}
  \mathbf{h}_{k}(\mathbf{x} + d \mathbf{x}) \approx \mathbf{h}_{k}(\mathbf{x}) + d \mathbf{x} \left. \frac{\partial \mathbf{h}_{k}(\mathbf{x})}{\partial \mathbf{x}} \right|_{\mathbf{x}}\,,
\end{equation}
...
The Jacobian matrices of the functionals $\bm{\phi}$ and $\mathbf{h}$, which are employed in the linearisation, are given by  
...
\begin{equation}\label{eq:apriori_estimate_extended}
  \hat{\mathbf{x}}^{-}_k = \bm{\phi}_{k-1}(\mathbf{x}_{k-1})\,,
\end{equation}
...
\begin{equation}\label{eq:predicted_measurement_extended}
  \hat{\mathbf{z}}_k = \mathbf{h}_{k}(\mathbf{x}^{-}_{k})\,.
\end{equation}
...
The a priori covariance matrix $\mathbf{P}^-_{k}$, the cooresponding Kalman gain $\mathbf{K}_{k}$, and the a posteriori covariance matrix $\mathbf{P}_{k}$ are equal to Equations \ref{eq:Kalman_gain_extended}, \ref{eq:apriori_error_cov}, and \ref{eq:aposteriori_error_cov} in Section \ref{sec:Kalman_equations} but reproduced here for convenience of presentation:
...
Figure \ref{fig:kalman_filter_cycle} illustrates the predict and update behaviour of the extended Kalman filter algorithm. 
...
\node [block, align=center] (init) {Initialisation of parameters \\ $\mathbf{P}_{0}, \mathbf{x}_{0}, \mathbf{H}^{[1]}_{0}, \bm{\Phi}^{[1]}_{0}, \mathbf{Q}_{0}, \mathbf{R}_{0},$};
\node [block, align=center, below of=init, node distance=3.2cm] (predict) {Time update (predict) \\ Compute \emph{a priori} estimate: \\ $\hat{\mathbf{x}}^{-}_k = \bm{\phi}_{k-1}(\mathbf{x}_{k-1})$ \\ Compute \emph{a priori} error covariance: \\ $\mathbf{P}^-_{k} = \bm{\Phi}^{[1]}_{k-1} \mathbf{P}_{k-1} \bm{\Phi}^{[1]T}_{k-1} + \mathbf{Q}_{k-1}$};
...
\end{tikzpicture}
\caption{Operation cycle of extended Kalman filter algorithm illustrating predict and correct behaviour.} \label{fig:kalman_filter_cycle}
\end{figure}

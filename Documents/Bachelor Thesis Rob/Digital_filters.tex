...
\noindent
where the index $k \in \mathbb{N}^0$ denotes discrete time normalised to the sampling interval. The observation or measurement vector $\mathbf{z}_k \in \mathbb{R}^m$ is given by
...
\noindent
The $n\times1$ vector $w_k$ and the $m\times1$ vector $v_k$ represent the process noise and the measurement noise, respectively, modelled as zero-mean, Gaussian white noise
...
\noindent
where $\mathbf{Q}_k$ is the process noise covariance matrix and $\mathbf{R}_k$ the measurement noise covariance matrix. The $n\times n$ matrix $\mathbf{\Phi}_{k}$ in \ref{eq:time_dynamical_system_plant} relates the state at the previous time step $k-1$ to the state at the current step $k$. The $n\times l$ matrix $\mathbf{B}_{k}$ relates the optional control input $\mathbf{u}_k \in \mathbb{R}^l$ to the state $\mathbf{x}_k$. Finally, the $m\times n$ matrix $\mathbf{H}_{k}$ relates the state $\mathbf{x}_k$ to the measurement $\mathbf{z}_k$.

The vector $\hat{\mathbf{x}}^-_k \in \mathbb{R}^n$ is defined as the a priori state estimate representing knowledge of the process prior to step $k$ and $\hat{\mathbf{x}}_k \in \mathbb{R}^n$ as the a posteriori state estimate at step $k$ given the measurement $\mathbf{z}_k$ given by
...
The Kalman filter equations can be divided into two groups: \emph{time update} equations \ref{eq:apriori_estimate} and \ref{eq:apriori_error_cov}, and the \emph{measurement update} Equations \ref{eq:Kalman_gain}, \ref{eq:estimation_kalman}, and \ref{eq:aposteriori_error_cov} as shown in Figure \ref{fig:kalman_filter_cycle}.

\tikzstyle{block} = [draw, rectangle, 
    minimum height=2cm, minimum width=10cm]
\tikzstyle{output} = [coordinate]
...
\node [block, align=left] (init) {Initialisation of parameters \\ $\mathbf{P}_{0}, \mathbf{x}_{0}, \mathbf{H}_{0}, \mathbf{\Phi}_{0}, \mathbf{Q}_{0}, \mathbf{R}_{0},$};
\node [block, align=left, below of=init, node distance=3.5cm] (predict) {Time update (predict) \\ Project state ahead (a priori estimate): \\ $\hat{\mathbf{x}}^-_k = \mathbf{\Phi}_{k-1}\hat{\mathbf{x}}_{k-1}+\mathbf{B}_{k-1}\mathbf{u}_{k-1}$ \\ Project error covariance ahead: \\ $\mathbf{P}^-_{k} = \mathbf{\Phi}_{k-1} \mathbf{P}_{k-1} \mathbf{\Phi}^T_{k-1} + \mathbf{Q}_{k-1}$};
\node [block, align=left, below of=predict] (update) {Measurement update (correct) \\ Compute Kalman gain: \\ $\mathbf{K}_{k} = \mathbf{P}^-_k \mathbf{H}^T_k[\mathbf{H}_k \mathbf{P}^-_k \mathbf{H}^T_k + \mathbf{R}_k]^{-1}$ \\ Update a priori estimate with measurement $\mathbf{z}_k$: \\ $\hat{\mathbf{x}}_k = \hat{\mathbf{x}}^-_k + \mathbf{K}_{k}[\mathbf{z}_k-\mathbf{H}_{k}\hat{\mathbf{x}}^-_k]$ \\ Update error covariance: \\ $\mathbf{P}_{k} = [\mathbf{I} - \mathbf{K}_{k}\mathbf{H}_{k}]\mathbf{P}^-_{k}$};
...
\node [output, below of=init, node distance=1.5cm, name=help4] {};
\node [output, right of=help4, node distance=5.6cm, name=help3] {};

\draw [draw,->, align=left] (init) -- (predict);
\draw [draw,->, align=left] (predict) -- (update);
\draw [draw,->, align=left] (update) -- node [label={left:Output}]{} (output);
\draw [draw,-] (help1) -- (help2);
\draw [draw,-] (help2) -- (help3);
\draw [draw,->] (help3) -- (help4);

\end{tikzpicture}
\caption{Operation cycle of Kalman filter algorithm.} \label{fig:kalman_filter_cycle}
\end{figure}

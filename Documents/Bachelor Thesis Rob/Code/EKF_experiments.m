...
pitch_KF_right_thigh = gw.fusion_KF(gyro_KF, obs_KF, f, ...
        var(pitch_acc_right_thigh), var(pitch_acc_right_thigh),...
        var(gyro_KF), opt_alpha_KF, opt_beta_KF, pitch_acc_right_thigh(1));
...
pitch_KF_right_shank = gw.fusion_KF(gyro_KF, obs_KF, f, ...
        var(pitch_acc_right_shank), var(pitch_acc_right_shank),...
        var(gyro_KF), opt_alpha_KF, opt_beta_KF, pitch_acc_right_shank(1));

%% Optimise EKF Prarameters

 % Set initial value of parameters;
rmse_EKF = 1;
p0 = [100, 0.01, 0.01, 0.0001, 1000, 1000, 10, 10];

 % Call the optimization routine.
[pmin, fmin, ct] = gw.optimize_EKF(g_Y_right_thigh_1_C', ...
                            g_Y_right_shank_1_C', ...
                            a_X_right_thigh_1_C', ...
                            a_Z_right_thigh_1_C', ...
                            a_X_right_shank_1_C', ...
                            a_Z_right_shank_1_C', ...
                            f, 0.35, 0.25, [pitch_QS_right_thigh; pitch_QS_right_shank], p0, rmse_EKF);
 %%                       
fprintf('--------------------KF OPTIMIZATION-----------------------\n')
    fprintf('The optimization process finished in %d iterations.\n', ct)
    fprintf('The minimum RMSE found is: %0.4f\n', fmin);

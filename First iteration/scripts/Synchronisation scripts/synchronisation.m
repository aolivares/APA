   ...
   % Set the firt and end index to extract the FP files names from excel.
    ...
    for i = 1:n_files
        ...
        [header, first_line, abta, count, col_first_block] = ...
            readFPHeader(fullfile('/APA/First iteration/data/ForcePlate/', ...
            filename_FP));
        ...
        fid = fopen(fullfile('/APA/First iteration/data/ForcePlate/',filename_FP));
...
% -------------------------------------------------------------------------
   ...
   % Set flags which control the visibility of the figures.
    ...
    load(fullfile('/APA/First iteration/data/GaitWatch/',char(filename_GW)));
        ...
        switch position_segment
            ...
            case 'center trunk'
                ...
                yaw_2KF_center_trunk = gw.fusion_KF(gz,yaw_mag_kf,f,...
                    var(yaw_mag_kf),var(yaw_mag_kf),var(gz),alpha_KF,...
                    beta_KF,yaw_mag_kf(1))';
                ...
                roll_GKF_center_trunk = gw.fusionGKF(gx,roll_acc,f,...
                    var(roll_acc),var(roll_acc),var(gx),alpha1,alpha2,...
                    beta1,beta2,roll_acc(1),marker_fsd)';
                yaw_GKF_center_trunk = gw.fusionGKF(gz,yaw_mag,f,...
                    var(yaw_mag),var(yaw_mag),var(gz),alpha1,alpha2,...
                    beta1,beta2,yaw_mag(1), marker_fsd)';
                yaw_2GKF_center_trunk = gw.fusionGKF(gz,yaw_mag_kf,f,...
                   var(yaw_mag_kf), var(yaw_mag_kf),var(gz),alpha1,...
                    alpha2,beta1,beta2,yaw_mag_kf(1),marker_fsd)';
                ...
                [roll_EKF_center_trunk, pitch_EKF_center_trunk, ...
                    yaw_EKF_center_trunk] = gw.quat9dofEKF(ax, ay, az,...
                     gx, gy, gz, hx, hy, -hz, gyroVarX, gyroVarY, ...
                   gyroVarZ, alpha, mu_gain, f, state_ini);           
    ...
    % Turn some signals into row vectors.
...
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
pitch_thighs_acc = createTimeseriesGW([pitch_acc_left_thigh; pitch_acc_right_thigh], ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Pitch thighs (computed from acceleration)', 'seconds', 'deg');
                         
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
pitch_shanks_acc = createTimeseriesGW([pitch_acc_left_shank; pitch_acc_right_shank], ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Pitch shanks (computed from acceleration)', 'seconds', 'deg');
                         
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
pitch_trunk_GKF = createTimeseriesGW(pitch_GKF_center_trunk, ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Pitch trunk (GKF)', 'seconds', 'deg');

% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
pitch_thighs_GKF = createTimeseriesGW([pitch_GKF_left_thigh], ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Pitch thighs (GKF)', 'seconds', 'deg');
                         
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
pitch_shanks_GKF = createTimeseriesGW([pitch_GKF_left_shank; pitch_GKF_right_shank], ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Pitch thighs (GKF)', 'seconds', 'deg');
                         
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
pitch_arms_gyro = createTimeseriesGW([pitch_gyro_left_arm; pitch_gyro_right_arm], ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Pitch arms (computed from gyroscope)', 'seconds', 'deg');
                         
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
pitch_thighs_gyro = createTimeseriesGW([pitch_gyro_left_thigh; pitch_gyro_right_thigh], ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Pitch thighs (computed from gyroscope)', 'seconds', 'deg');
                         
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
pitch_shanks_gyro = createTimeseriesGW([pitch_gyro_left_shank; pitch_gyro_right_shank], ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Pitch shanks (computed from gyroscope)', 'seconds', 'deg');
                         
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
pitch_trunk_KF = createTimeseriesGW(pitch_KF_center_trunk, ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Pitch trunk (KF)', 'seconds', 'deg');
                         
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
pitch_thighs_KF = createTimeseriesGW([pitch_KF_left_thigh; pitch_KF_right_thigh], ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Pitch thighs (KF)', 'seconds', 'deg');
                         
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
pitch_shanks_KF = createTimeseriesGW([pitch_KF_left_shank; pitch_KF_right_shank], ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Pitch trunk (KF)', 'seconds', 'deg');
                         
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
roll_trunk_GKF = createTimeseriesGW(roll_GKF_center_trunk, ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Roll trunk (GKF)', 'seconds', 'deg');
                         
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
roll_trunk_KF = createTimeseriesGW(roll_KF_center_trunk, ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Roll trunk (KF)', 'seconds', 'deg');
                         
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
roll_arms_GKF = createTimeseriesGW([roll_gyro_left_arm, roll_gyro_right_arm], ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Roll arms (gyroscope)', 'seconds', 'deg');
                         
% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
yaw_trunk_GKF = createTimeseriesGW(yaw_GKF_center_trunk, ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Yaw trunk (GKF)', 'seconds', 'deg');

% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
yaw_trunk_2GKF = createTimeseriesGW(yaw_2GKF_center_trunk, ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Yaw trunk (2GKF)', 'seconds', 'deg');

% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
yaw_trunk_KF = createTimeseriesGW(yaw_KF_center_trunk, ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
                             'Yaw trunk (KF)', 'seconds', 'deg');

% Create cell array containing the time series of the seperate cycles of 
% the angular rate of the arms.
yaw_trunk_2KF = createTimeseriesGW(yaw_2KF_center_trunk, ...
                             time_GW, sync_peaks, last_peaks, add_samples, ...
...
% -------------------------------------------------------------------------
% Store cell arrays of time series objects as .mat file
% -------------------------------------------------------------------------
...
% Create the output file name.
 ...
 save(['/Users/Rob/Documents/APA/First iteration/data/Synchronised/' ...

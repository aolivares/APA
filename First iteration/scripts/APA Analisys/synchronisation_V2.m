...
for i = 3:rows-1
        ...
        if  isempty( char(strtrim(file_excel(i,4))) ) == 0  
            ...
            % Also, we extract the parameters for the activity detection.
            threshold_ltsd_right = strtrim(file_excel(i,11));
            win_ltsd_right = strtrim(file_excel(i,12));
            threshold_ltsd_left = strtrim(file_excel(i,13));
            win_ltsd_left = strtrim(file_excel(i,14));
...
showPlotsGyroShank ...
threshold_ltsd_right win_ltsd_right threshold_ltsd_left win_ltsd_left
...
lwin_ltsd = str2num(win_ltsd_right{1,1});   
threshold_ltsd = str2num(threshold_ltsd_right{1,1}); 
shift_ltsd = 10;
...
% LTSD (window size, decision threshold and overlapping).
lwin_ltsd = str2num( win_ltsd_left{1,1});
threshold_ltsd = str2num( threshold_ltsd_left{1,1});   
shift_ltsd = 10;
...
% Calculate the mean of the difference between the peaks found for the
% synchronisastion. Also, the result is divided by 200, the sample
% frequency for obtaining the difference in time units.
dif_peaks_position= mean(abs(sync_peaks_acc-sync_peaks_gyro))/200;

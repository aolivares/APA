...
% -------------------------------------------------------------------------
    ...
    fprintf(['\nRead data from text files. Patient ', num2str(j), ' of ', num2str(length(index_GW)-1), '\n\n']);
   ...
   % Set the firt and end index to extract the FP files names from excel.
    ...
    for i = 1:n_files
        ...
        [header, first_line, abta, count, col_first_block] = ...
            readFPHeader(fullfile('/APA/First iteration/data/ForcePlate/Raw/',filename_FP));

        % Read formatted data from text file. As always "fid" is a file 
        % identifier that we obtain with "fopen".
        fid = fopen(fullfile('/APA/First iteration/data/ForcePlate/Raw/', filename_FP));
...
% -------------------------------------------------------------------------
    ...
    fprintf(['\nCalibration in progress. Patient ', num2str(j), ' of ', num2str(length(index_GW)-1), '\n\n']);
   ...
   % Set flags which control the visibility of the figures.
    ...
    load(fullfile('/APA/First iteration/data/GaitWatch/Raw/',char(filename_GW)));
...
% acceleration trunk.
a_trunk_ts = createTimeseriesGW([a_X_center_trunk_3_C; a_Y_center_trunk_3_C; ...
...
% Create cell array containing the time series of the seperate cycles of 
% acceleration thigh.
a_thigh_ts = createTimeseriesGW([a_X_left_thigh_1_C;  a_Z_left_thigh_1_C; ...
...
% Create cell array containing the time series of the seperate cycles of 
% acceleration shank.
a_shank_ts = createTimeseriesGW([a_X_left_shank_1_C;  a_Z_left_shank_1_C; ...
...
% Create the output file name.
 ...
 save(['/Users/Rob/Documents/APA/First iteration/data/Synchronised/' ...
       name_file], 'a_trunk_ts','a_shank_ts', 'a_thigh_ts', ...
      'force_sensors_ts');  
